# RPVM方法使用指南

## 📋 目录

1. [快速开始](#快速开始)
2. [详细配置](#详细配置)
3. [运行实验](#运行实验)
4. [结果分析](#结果分析)
5. [常见问题](#常见问题)
6. [高级用法](#高级用法)

---

## 🚀 快速开始

### 步骤1: 环境准备

```bash
# 确保在flashRAG环境中
conda activate flashRAG  # 或你的环境名

# 安装必要的依赖
pip install openai tiktoken
```

### 步骤2: 配置OpenAI API

```bash
# Linux/Mac
export OPENAI_API_KEY='your-api-key-here'

# Windows PowerShell
$env:OPENAI_API_KEY='your-api-key-here'

# Windows CMD
set OPENAI_API_KEY=your-api-key-here
```

### 步骤3: 配置文件路径

编辑 `rpvm_config.yaml`：

```yaml
# 修改以下路径为你的实际路径
data_dir: "D:/path/to/your/datasets"  # FlashRAG数据目录
retrieval_model_path: "D:/path/to/e5-base-v2"  # E5模型路径
index_path: "D:/path/to/e5_Flat.index"  # E5索引路径
corpus_path: "D:/path/to/general_knowledge.jsonl"  # 语料库路径
```

### 步骤4: 测试运行

```bash
cd RPVM

# 运行基本测试
python test_rpvm.py

# 运行简单示例
python simple_example.py --mode simple
```

---

## ⚙️ 详细配置

### 配置文件结构

`rpvm_config.yaml` 包含以下几个部分：

#### 1. 基本设置

```yaml
data_dir: "datasets"           # 数据集根目录
save_dir: "RPVM/output"       # 输出目录
seed: 2024                    # 随机种子
save_intermediate_data: True  # 是否保存中间数据
gpu_id: "0"                   # GPU设备ID，null表示使用CPU
```

#### 2. 数据集设置

```yaml
dataset_name: "hotpotqa"  # 支持: hotpotqa, 2wikimultihopqa
split: ["test"]           # 数据集分割: train, dev, test
```

#### 3. 检索器设置 (E5)

```yaml
retrieval_method: "e5"
retrieval_model_path: "intfloat/e5-base-v2"  # 本地路径或HuggingFace ID
index_path: "indexes/e5_Flat.index"          # FAISS索引文件
corpus_path: "indexes/general_knowledge.jsonl"  # 文档语料库
retrieval_topk: 5              # 每次检索返回文档数
retrieval_batch_size: 32       # 检索批次大小
retrieval_use_fp16: True       # 使用FP16加速
```

#### 4. 生成器设置 (OpenAI)

```yaml
framework: "openai"
generator_model: "gpt-3.5-turbo"  # 支持: gpt-3.5-turbo, gpt-4, etc.
openai_setting:
  api_key: null                   # API密钥(null表示从环境变量读取)
  base_url: "https://api.openai.com/v1"  # API基础URL
generator_batch_size: 1           # 生成批次大小
generation_params:
  max_tokens: 512                 # 最大生成token数
  temperature: 0.7                # 生成温度
  top_p: 0.9                      # nucleus sampling参数
```

#### 5. RPVM特定参数

```yaml
rpvm_config:
  max_iter: 5                      # 最大迭代次数
  max_retrieval_attempts: 2        # 每个plan最大检索重试次数
  retrieval_topk: 5                # 每次检索返回文档数
  memory_max_tokens: 3000          # 记忆最大token限制
  enable_memory_summary: True      # 启用记忆自动摘要
  planner_temperature: 0.7         # 规划器温度(控制计划生成的随机性)
  verifier_temperature: 0.3        # 验证器温度(较低=更保守的判断)
  final_answer_temperature: 0.5    # 最终答案温度
```

### 参数调优建议

| 参数 | 推荐值 | 说明 |
|------|--------|------|
| `max_iter` | 3-7 | 简单问题用3-5，复杂问题用5-7 |
| `max_retrieval_attempts` | 2-3 | 增加可提高召回，但会增加API调用 |
| `retrieval_topk` | 5-10 | 增加可提供更多证据，但会增加上下文长度 |
| `planner_temperature` | 0.5-0.9 | 较高值产生更多样的计划 |
| `verifier_temperature` | 0.1-0.5 | 较低值使验证更严格 |
| `memory_max_tokens` | 2000-4000 | 根据模型上下文窗口调整 |

---

## 🎯 运行实验

### 1. 小规模测试

先在少量样本上测试，确保配置正确：

```bash
# 测试5个样本
python run_rpvm_exp.py \
    --dataset_name hotpotqa \
    --split test \
    --num_samples 5 \
    --gpu_id 0
```

### 2. 完整实验

#### HotpotQA数据集

```bash
python run_rpvm_exp.py \
    --dataset_name hotpotqa \
    --split test \
    --gpu_id 0
```

#### 2WikiMultihopQA数据集

```bash
python run_rpvm_exp.py \
    --dataset_name 2wikimultihopqa \
    --split test \
    --gpu_id 0
```

### 3. 自定义OpenAI API

```bash
python run_rpvm_exp.py \
    --dataset_name hotpotqa \
    --split test \
    --gpu_id 0 \
    --openai_api_key your-api-key
```

### 4. CPU运行

如果没有GPU或GPU内存不足：

```bash
# 方法1: 命令行指定
python run_rpvm_exp.py \
    --dataset_name hotpotqa \
    --split test \
    --gpu_id null

# 方法2: 修改配置文件
# 在rpvm_config.yaml中设置: gpu_id: null
```

---

## 📊 结果分析

### 输出文件

实验完成后，在 `RPVM/output/rpvm_experiments/` 目录下生成：

#### 1. `intermediate_data.jsonl`

每行一个样本的完整推理过程：

```json
{
  "question": "What is the fight song of the university in Lawrence, Kansas?",
  "iterations": [
    {
      "iteration": 1,
      "plans": [
        "The university in Lawrence, Kansas is the University of Kansas.",
        "The fight song of the University of Kansas is 'I'm a Jayhawk'."
      ],
      "verifications": [
        {
          "plan_index": 1,
          "verdict": "supported",
          "retrievals": 1
        },
        {
          "plan_index": 2,
          "verdict": "contradicted",
          "corrected_plan": "The fight song is 'Kansas Song'.",
          "retrievals": 1
        }
      ],
      "updated_memory": "1. University of Kansas (verified)\n2. Fight song is Kansas Song (corrected)"
    }
  ],
  "final_answer": "Kansas Song",
  "total_retrievals": 2
}
```

#### 2. `metric_score.txt`

评估指标：

```
EM: 0.452
F1: 0.589
ACC: 0.623
```

#### 3. `config.yaml`

保存的完整配置，便于复现

### 分析中间数据

使用Python分析中间数据：

```python
import json

# 读取中间数据
with open('output/rpvm_experiments/intermediate_data.jsonl') as f:
    data = [json.loads(line) for line in f]

# 统计迭代次数
iterations = [len(item['iterations']) for item in data]
print(f"平均迭代次数: {sum(iterations)/len(iterations):.2f}")

# 统计检索次数
retrievals = [item['total_retrievals'] for item in data]
print(f"平均检索次数: {sum(retrievals)/len(retrievals):.2f}")

# 统计verdict分布
verdicts = []
for item in data:
    for iter_info in item['iterations']:
        for ver in iter_info.get('verifications', []):
            verdicts.append(ver['verdict'])

from collections import Counter
print("Verdict分布:", Counter(verdicts))
```

### 可视化分析

创建简单的统计图表：

```python
import matplotlib.pyplot as plt
import json

# 读取数据
with open('output/rpvm_experiments/intermediate_data.jsonl') as f:
    data = [json.loads(line) for line in f]

# 绘制迭代次数分布
iterations = [len(item['iterations']) for item in data]
plt.figure(figsize=(10, 4))
plt.hist(iterations, bins=range(1, max(iterations)+2), alpha=0.7)
plt.xlabel('Iteration Count')
plt.ylabel('Frequency')
plt.title('Distribution of Iteration Counts')
plt.savefig('iteration_distribution.png')

# 绘制检索次数分布
retrievals = [item['total_retrievals'] for item in data]
plt.figure(figsize=(10, 4))
plt.hist(retrievals, bins=20, alpha=0.7)
plt.xlabel('Retrieval Count')
plt.ylabel('Frequency')
plt.title('Distribution of Retrieval Counts')
plt.savefig('retrieval_distribution.png')
```

---

## ❓ 常见问题

### Q1: OpenAI API调用失败

**错误**: `openai.error.AuthenticationError`

**解决**:
1. 检查API key是否正确
2. 检查API key是否有余额
3. 检查网络连接是否正常
4. 尝试设置代理（如需要）

```python
# 在配置文件中添加代理设置
openai_setting:
  api_key: "your-key"
  base_url: "https://api.openai.com/v1"
  http_client: 
    proxies: "http://your-proxy:port"
```

### Q2: 检索器加载慢或失败

**问题**: E5模型或索引加载时间长

**解决**:
1. 确保模型和索引在本地（不要每次从HuggingFace下载）
2. 使用SSD存储索引文件
3. 设置 `retrieval_use_fp16: True` 减少内存占用
4. 如果是首次运行，可能需要下载模型

```bash
# 预先下载E5模型
from transformers import AutoModel, AutoTokenizer
model = AutoModel.from_pretrained("intfloat/e5-base-v2")
tokenizer = AutoTokenizer.from_pretrained("intfloat/e5-base-v2")
model.save_pretrained("./models/e5-base-v2")
tokenizer.save_pretrained("./models/e5-base-v2")
```

### Q3: GPU内存不足

**错误**: `CUDA out of memory`

**解决**:
1. 减小batch size
2. 使用FP16
3. 使用CPU
4. 清理GPU缓存

```python
import torch
torch.cuda.empty_cache()
```

### Q4: 数据集加载失败

**错误**: `Dataset not found`

**解决**:
1. 检查 `data_dir` 路径是否正确
2. 确认数据集文件存在
3. 检查数据集名称拼写
4. 参考FlashRAG文档准备数据集

### Q5: 结果与预期不符

**可能原因**:
1. 检索质量不高 → 调整 `retrieval_topk`
2. 计划生成质量差 → 调整 `planner_temperature`
3. 验证过于宽松/严格 → 调整 `verifier_temperature`
4. 迭代次数不够 → 增加 `max_iter`

---

## 🔧 高级用法

### 1. 自定义Prompt模板

修改 `rpvm_pipeline.py` 中的prompt构建方法：

```python
def _build_planner_prompt(self, question: str, memory: str) -> str:
    # 自定义你的prompt逻辑
    custom_prompt = f"""Your custom prompt here
    Question: {question}
    Memory: {memory}
    """
    return custom_prompt
```

### 2. 使用不同的LLM

虽然当前实现使用OpenAI API，但可以扩展支持其他LLM：

```yaml
# 在配置文件中
framework: "hf"  # 或 "vllm"
generator_model: "meta-llama/Llama-2-7b-chat-hf"
generator_model_path: "path/to/llama2"
```

需要修改 `rpvm_pipeline.py` 中的消息格式以适配不同模型。

### 3. 集成Reranker

提高检索质量：

```yaml
use_reranker: True
rerank_model_name: "bge-reranker-base"
rerank_model_path: "BAAI/bge-reranker-base"
```

### 4. 批量实验对比

创建实验脚本对比不同配置：

```python
experiments = [
    {"max_iter": 3, "note": "iter3"},
    {"max_iter": 5, "note": "iter5"},
    {"max_iter": 7, "note": "iter7"},
]

for exp in experiments:
    config_dict = {
        "rpvm_config": {"max_iter": exp["max_iter"]},
        "save_note": f"rpvm_{exp['note']}"
    }
    # 运行实验...
```

### 5. 自定义评估指标

添加自定义评估：

```python
from flashrag.evaluator import Evaluator

class CustomEvaluator(Evaluator):
    def evaluate(self, dataset):
        # 自定义评估逻辑
        custom_metric = self.calculate_custom_metric(dataset)
        return {"custom": custom_metric, **super().evaluate(dataset)}
```

### 6. 错误处理和重试

添加更robust的错误处理：

```python
# 在rpvm_pipeline.py中添加
import time
from tenacity import retry, stop_after_attempt, wait_exponential

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
def _call_generator_with_retry(self, messages, **kwargs):
    try:
        return self.generator.generate([messages], **kwargs)[0]
    except Exception as e:
        print(f"Generator call failed: {e}, retrying...")
        raise
```

---

## 📈 性能优化

### 1. 并行处理

虽然当前实现是串行的，但可以通过以下方式并行化：

```python
from concurrent.futures import ThreadPoolExecutor

def process_batch(questions):
    with ThreadPoolExecutor(max_workers=4) as executor:
        results = list(executor.map(pipeline._run_single_question, questions))
    return results
```

### 2. 缓存检索结果

```yaml
save_retrieval_cache: True
use_retrieval_cache: True  # 在第二次运行时设置
retrieval_cache_path: "output/retrieval_cache.json"
```

### 3. 减少API调用

- 增加 `planner_temperature` 使生成更稳定
- 减少 `max_retrieval_attempts` 
- 启用 `enable_memory_summary` 避免过长上下文

---

## 📚 参考资源

- [FlashRAG文档](../docs/)
- [E5模型](https://huggingface.co/intfloat/e5-base-v2)
- [OpenAI API文档](https://platform.openai.com/docs)
- [HotpotQA数据集](https://hotpotqa.github.io/)
- [2WikiMultihopQA数据集](https://github.com/Alab-NII/2wikimultihop)

---

## 💡 最佳实践

1. **从小规模开始**: 先用5-10个样本测试
2. **监控API消耗**: 注意OpenAI API的token使用和成本
3. **保存中间结果**: 启用 `save_intermediate_data` 便于调试
4. **版本控制配置**: 每次实验保存配置文件
5. **记录实验日志**: 记录不同配置下的性能表现
6. **定期备份**: 备份重要的实验结果和索引文件

---

如有问题，请参考 [README.md](README.md) 或查看 [常见问题](#常见问题) 部分。
