...
使用 wiki18 小语料库 + ChatGPT-3.5-turbo
"""
import os
import sys
import argparse
...
def run_openai_experiment(args):
    """运行OpenAI API RPVM实验"""
    
    # 设置保存标识
    save_note = f"rpvm_openai_{args.split}"
    ...
    print(f"all_split 内容: {all_split}")
    ...
    # 创建RPVM Pipeline
    print("正在初始化 RPVM Pipeline (使用 OpenAI API)...")
...
def main():
    parser = argparse.ArgumentParser(description="运行OpenAI API RPVM实验")
