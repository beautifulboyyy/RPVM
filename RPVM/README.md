...
RPVM是一个迭代推理框架，通过"反思-行动-记忆"(Reflect-Act-Remember)的方式解决多跳问题：

1. **Reflective Planner (反思规划器)**: 基于问题和当前记忆生成推理计划链
2. **Plan Verifier (计划验证器)**: 通过检索文档验证每个计划，判断其是否被支持、矛盾或证据不足
3. **Memory Updater (记忆更新器)**: 将验证过的计划加入记忆，支持后续推理
